...
resource "azurerm_network_security_group" "automation" {
  ...
  security_rule {
    ...
    destination_address_prefix = data.azurerm_subnet.automation.address_prefixes[0]
  }
...
resource "azurerm_subnet_network_security_group_association" "automation" {
  ...
  network_security_group_id = azurerm_network_security_group.automation.id
}
