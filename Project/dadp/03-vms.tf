...
resource "azurerm_network_interface" "dadpvms" {
...
resource "azurerm_linux_virtual_machine" "dadpvms" {
  ...
  network_interface_ids = [azurerm_network_interface.dadpvms[each.key].id]
...
resource "azurerm_windows_virtual_machine" "dadpvms" {

  for_each = { for vm in var.virtualmachines : vm.hostname => vm
    if vm.offer == "WindowsServer"
  }
  ...
  admin_username = "admin_win"
  admin_password = "welkom@1234!"
...
resource "azurerm_virtual_machine_data_disk_attachment" "datadisk_01" {
  ...
  virtual_machine_id = azurerm_linux_virtual_machine.dadpvms[each.key].id
  lun                = "10"
  caching            = "ReadWrite"
